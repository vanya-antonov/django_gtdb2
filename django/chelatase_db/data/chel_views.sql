...
select f.id, f.seq_id, s.org_id,
    ...
    (select t.value from feat_params t where t.parent_id=f.id and t.name='chel_gene_group') AS chel_gene_group,
    (select t.value from feat_params t where t.parent_id=f.id and t.name='chel_pathway') AS chel_pathway,
...
select o.id, o.name, o.genus, o.phylum, o.kingdom,
    ...
    (select count(distinct t.id) from chel_feats_v t where t.org_id=o.id and t.chel_gene='cobT' and t.fs_len in (-1,1)) AS num_cobT_fs,
    (select count(distinct t.id) from chel_feats_v t where t.org_id=o.id and t.chel_gene='chlD' and t.fs_len in (-1,1)) AS num_chlD_fs,
    (select count(distinct t.id) from chel_feats_v t where t.org_id=o.id and t.chel_gene='bchD' and t.fs_len in (-1,1)) AS num_bchD_fs,
